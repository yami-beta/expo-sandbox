...
- 本プロジェクトのサンプル実装: ホームタブ「ナビゲーションパターン」から開くと、各値の素の挙動を iOS / Android で比較できる。**presentation 値ごとに on-root / in-tab の 2 配置を用意し、中間ページから両者を開いて挙動差を比較できる構成**。
  - ディレクトリ構造:
    ```
    apps/sandbox/src/app/(tabs)/(home)/navigation-patterns/
      index.tsx                                  # 一覧
      <value>/index.tsx                          # 中間ページ (on-root / in-tab を提示)
      <value>/in-tab.tsx                         # タブ内 Stack 配下サンプル
    apps/sandbox/src/app/navigation-patterns/
      <value>/on-root.tsx                        # ルート Stack 配下サンプル
    ```
  - `<value>` は `card`, `modal`, `transparent-modal`, `contained-modal`, `contained-transparent-modal`, `full-screen-modal`, `form-sheet` の 7 種。全 14 サンプル画面 (7 値 × 2 配置)。
  - 動線: 一覧 → presentation 値別の中間ページ → on-root / in-tab の 2 リンクから個別サンプルへ。
- **サンプルは観察用、推奨配置の選定指針は別**: 14 サンプル全部が推奨配置というわけではない。実プロダクトでの配置指針は [`screen-options.md`](./screen-options.md) を参照 (`modal` / `formSheet` はルート、`containedModal` / `containedTransparentModal` は例外的にタブ内側に置くのが推奨)。特に **`containedModal/on-root` と `containedTransparentModal/on-root` は「ルートに置くと意味が消える」フォールバック挙動を観察するためのサンプル** であって、推奨配置ではない。
