...
- 本プロジェクトのサンプル実装: `apps/sandbox/src/app/navigation-patterns/` 配下に `presentation` 値ごとのリファレンス画面が並んでいる。ホームタブ「ナビゲーションパターン」から開くと、各値の素の挙動を iOS / Android で比較できる。
  - `card.tsx` (ホーム Stack 配下) → デフォルト push
