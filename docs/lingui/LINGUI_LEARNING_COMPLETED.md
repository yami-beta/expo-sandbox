...
- [x] エラーコードと翻訳キーのマッピング
  - [x] エラーコード体系の設計（V001-V005, N001-N005, S001-S003）
  - [x] 型安全なエラーハンドリング関数
  - [x] エラーメッセージのテンプレート化
- [x] バリデーションメッセージの一元化
  - [x] フォームバリデーションエラーの管理
...
- [x] ネットワークエラーの適切な表示
  - [x] 接続エラー、タイムアウトの翻訳
  - [x] HTTPステータスコード別メッセージ
  - [x] リトライ可能エラーの表示

### フェーズ13-C: 通知とアラートの翻訳（13日目-C）✅
- [x] プッシュ通知のローカライズ（シミュレーション）
  - [x] 通知タイトルとボディの翻訳パターン
  - [x] 動的な値を含む通知（ユーザー名、件数等）
  - [x] 通知カテゴリー別の翻訳
- [x] アプリ内通知の対応
  - [x] トースト通知の実装と翻訳
  - [x] スナックバーメッセージの国際化
...
### フェーズ14: パフォーマンス最適化（14日目）【実装中】✅
- [x] ~~バンドルサイズの分析と最適化~~ → ハードコード分析は実用的でないため削除
  - [x] ~~Expo Atlas（SDK 51+）を使用したバンドル分析~~
  - [x] ~~未使用翻訳の検出ツール作成~~
  - [x] ~~Tree shakingの最適化検証~~
  - [x] ~~翻訳ファイルのサイズ測定と分析~~
- [x] 遅延読み込みの実装
  - [x] `msg`マクロを使用した遅延翻訳パターンの実装（`lazy-translation.tsx`）
  - [x] 国選択ドロップダウンとステータス表示システムのサンプル
  - [x] Linguiの遅延ロードメカニズムの解説セクション追加
  - [x] React Nativeでの制限事項と実際に効果があるケースの説明
- [x] ランタイムパフォーマンスの改善
  - [x] `useMemo`/`useCallback`を使用した翻訳のメモ化（`optimization.tsx`）
  - [x] FlatListと組み合わせた大量アイテムの効率的な表示
  - [x] メモ化の有効/無効を切り替えて効果を確認できるデモ
  - [x] 検索フィルターと統計情報の動的更新

### フェーズ15: 高度な国際化対応（未実施）
※ フェーズ15は直近必要性が低いため実施を見送りました。将来的に必要になった場合は以下の項目を実装予定です。

- [ ] 右から左（RTL）言語のサポート準備
  - [ ] `I18nManager`を使用したRTL検出の実装
  - [ ] app.jsonに`supportsRTL: true`設定の追加
  - [ ] RTL対応レイアウトコンポーネントの作成
  - [ ] アラビア語サンプル実装（基本的な挨拶・メニュー）
  - [ ] flexDirection、textAlign、marginの自動調整
- [ ] アクセシビリティ対応
  - [ ] 翻訳されたアクセシビリティラベルの実装
  - [ ] スクリーンリーダー対応の改善
  - [ ] 音声ガイダンスのローカライズ
  - [ ] accessibilityLabel、accessibilityHintの国際化
  - [ ] VoiceOver（iOS）/TalkBack（Android）でのテスト
- [ ] 地域別カスタマイズ
  - [ ] 地域コード（ja-JP、en-US、en-GB等）への対応
  - [ ] 地域別表現の管理システム構築
  - [ ] 日付・通貨の地域別フォーマット拡張
  - [ ] 法的要件への対応準備（GDPR等）
  - [ ] 文化的な配慮事項のドキュメント化
...
### フェーズ13-B完了時のメモ（2025-08-20）
- **実装内容**：
  - `src/app/lingui-examples/error-handling-examples.tsx`に全機能を集約
  - 学習用サンプルとして1ファイルで完結する設計
  - 3つのセクション：エラーコード管理、フォームバリデーション、APIエラーハンドリング
- **エラーコード体系**：
  - バリデーション（V001-V005）：必須項目、形式、長さ、範囲など
  - ネットワーク（N001-N005）：接続、タイムアウト、404、401、500エラー
  - システム（S001-S003）：システムエラー、権限、ストレージ
- **技術的な実装**：
  - `ErrorInfo`型でエラー情報を構造化
  - `getErrorMessage`関数で動的な値（フィールド名、数値）を含むメッセージ生成
  - リトライ可能エラーの判定とUI表示
- **フォームバリデーション**：
  - リアルタイムバリデーション（onBlur後）
  - フィールド別エラー管理
  - 複数エラーの同時表示
- **APIエラーデモ**：
  - HTTPステータスコード別のエラーシミュレーション
  - リトライ機能の実装
  - エラー種別に応じた適切なメッセージ表示
- **翻訳追加**：
  - 54個の新規翻訳メッセージ
  - 全411メッセージ、英語翻訳率100%達成
- **学習ポイント**：
  - エラーハンドリングの体系的な管理方法
  - Linguiでの動的な値の埋め込み（`t\`${field}は必須です\`）
  - 実践的なバリデーションパターン

### フェーズ13-C完了時のメモ（2025-08-20）
- **実装内容**：
  - `src/app/lingui-examples/notification-alert-examples.tsx`に全機能を集約
  - 学習用サンプルとして1ファイルで完結する設計
  - 3つのセクション：プッシュ通知、トースト通知、システムアラート
- **プッシュ通知のシミュレーション**：
  - 4種類の通知タイプ（message、follow、like、system）
  - 動的な値（ユーザー名、通知数）を含む通知生成
  - タイトルとボディの動的生成関数
  - pluralマクロによる件数表示
- **トースト通知**：
...
- **システムアラート**：
  - 権限リクエスト（カメラ、位置情報、通知）
  - システム制限通知（ストレージ、ネットワーク、バッテリー）
  - React Native Alertとの統合
  - 権限状態の視覚的表示
- **技術的な実装**：
  - 通知タイプごとのメッセージ生成関数
  - TypeScriptによる型安全な実装
  - インタラクティブなUIコンポーネント
  - exactOptionalPropertyTypesに対応した型定義
- **翻訳追加**：
  - 58個の新規翻訳メッセージ
  - 全469メッセージ、英語翻訳率100%達成
- **学習ポイント**：
  - 通知システムの国際化パターン
  - 動的コンテンツを含む通知の翻訳
  - システムアラートとLinguiの統合方法
  - plural/t/Transの使い分け
...
├── lazy-translation.tsx    # 遅延翻訳パターンの実装例
└── optimization.tsx        # パフォーマンス最適化のサンプル
```

### 実装内容

#### 遅延翻訳パターン（lazy-translation.tsx）
- `msg`マクロによる遅延翻訳の実装例
- 国選択ドロップダウン（日本、アメリカ、イギリス、フランス、ドイツ）
- ステータス表示システム（処理待ち、処理中、完了、失敗）
- Linguiの遅延ロードメカニズムの解説
- React Nativeでの制限事項と実際の効果

#### 最適化サンプル（optimization.tsx）
- FlatListによる100件のアイテム表示
- `useMemo`による検索結果と統計情報のキャッシュ
- `useCallback`によるコールバック関数の安定化
- `React.memo`によるリストアイテムの再レンダリング防止
- メモ化の有効/無効切り替えデモ
- HeaderSectionのmemo化解除（FlatListの内部最適化により不要）

### 技術的な学習ポイント
- `msg`マクロ: MessageDescriptorを返し、`i18n._()`呼び出しまで翻訳を遅延
- React Nativeでは動的インポート不可、全翻訳ファイルが初期バンドルに含まれる
- FlatListのListHeaderComponentは内部最適化により自動的にフォーカスを保持
- メモ化技術の適切な使用方法と効果の可視化

---

## フェーズ15: 高度な国際化対応 - 実装詳細

### 作成予定ファイル
```
apps/sandbox/src/app/advanced-i18n/
├── rtl-examples.tsx         # RTL言語対応のサンプル
├── accessibility-examples.tsx # アクセシビリティの国際化
├── regional-examples.tsx    # 地域別カスタマイズのデモ
└── README.md               # 高度な国際化ガイド
```
...
// 翻訳されたアクセシビリティラベル
<Button
  accessibilityLabel={t`削除ボタン`}
  accessibilityHint={t`タップしてアイテムを削除します`}
...
### 計画調査メモ（2025-08-20）
- **フェーズ14の調査結果**：
  - 現在の翻訳ファイル: 469メッセージ、日本語77KB、英語72KB
  - Expo Atlas（SDK 51+）がバンドル分析に推奨
  - React Nativeでは言語別動的インポートに制限があるため、メッセージレベルの最適化が現実的
  - `msg`マクロを使用した遅延翻訳パターンが有効
- **フェーズ15の調査結果**：
  - RTL対応は`I18nManager`とapp.jsonの設定で実現可能
  - `supportsRTL: true`の設定が必要
  - アクセシビリティ属性の翻訳はLinguiの標準機能で対応可能
  - 地域コードの対応には追加実装が必要

### フェーズ14完了時のメモ（2025-08-21）
- **実装方針の変更**：
  - 当初計画のバンドル分析機能は、ハードコードされた統計表示になってしまうため削除
  - 実践的で学習価値の高いサンプル実装に注力
- **遅延翻訳パターン（`lazy-translation.tsx`）**：
  - `msg`マクロの実装例：国選択ドロップダウン、ステータス表示システム
  - Linguiの遅延ロードメカニズムの詳細解説を追加
  - React Nativeでの制限事項を明確化（動的インポート不可、バンドルサイズ削減効果なし）
  - 実際に効果があるケース：条件付き翻訳、初期化コスト削減、メモリ最適化、タブ/モーダルの遅延初期化
- **最適化サンプル（`optimization.tsx`）**：
  - 100件のアイテムを含むFlatListで最適化効果をデモ
  - `useMemo`、`useCallback`、`React.memo`の実践的な使用例
  - メモ化の有効/無効切り替えで効果を視覚的に確認可能
  - 検索フィルターと統計情報のリアルタイム更新
- **技術的な課題と解決**：
  - VirtualizedList入れ子問題 → FlatListのListHeaderComponent/ListFooterComponentで解決
  - TextInputフォーカス喪失問題 → HeaderSectionのmemo化で解決（後に解除して学習用に）
  - ESLintエラー（displayName、未使用変数） → 適切な関数名とクリーンアップ
- **HeaderSectionのmemo化について**：
  - 当初memo化していたが、FlatListの内部最適化により不要と判明
  - FlatListのListHeaderComponentは自動的に最適化される
  - React Nativeのreconciliationが既存インスタンスを保持
  - 学習目的でmemo化を解除し、コメントで理由を説明
- **翻訳追加**：
  - 新規メッセージ多数追加（「カテゴリー」など）
  - 全530メッセージ、英語翻訳率100%達成
- **学習ポイント**：
  - msg vs t マクロの使い分け（遅延 vs 即時翻訳）
  - React Nativeにおけるパフォーマンス最適化の現実的なアプローチ
  - FlatListの内部最適化メカニズムの理解
...
このドキュメントは作業記録として`docs/lingui/`ディレクトリに保管されます。

---

最終更新日: 2025-08-21
- フェーズ14: パフォーマンス最適化を完了（実践的なサンプル実装に注力）
- 学習プロジェクト完了、ドキュメントを作業記録として移動
