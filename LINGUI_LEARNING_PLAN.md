...
### フェーズ14: パフォーマンス最適化（14日目）【計画中】
- [ ] バンドルサイズの分析と最適化
  - [ ] Expo Atlas（SDK 51+）を使用したバンドル分析
  - [ ] 未使用翻訳の検出ツール作成
  - [ ] Tree shakingの最適化検証
  - [ ] 翻訳ファイルのサイズ測定と分析（現状: 日本語77KB、英語72KB、469メッセージ）
- [ ] 遅延読み込みの実装
  - [ ] `msg`マクロを使用した遅延翻訳パターンの実装
  - [ ] コンポーネントレベルでの翻訳最適化
  - [ ] メッセージのグループ化とチャンク戦略
  - [ ] ※React Nativeでは言語別動的インポートに制限があるため、メッセージレベルの最適化に注力
- [ ] ランタイムパフォーマンスの改善
  - [ ] `useMemo`/`useCallback`を使用した翻訳のメモ化
  - [ ] 高頻度使用翻訳のキャッシング戦略
  - [ ] レンダリング最適化のベストプラクティス実装
  - [ ] パフォーマンス計測とボトルネック分析
...
### バンドル分析コマンド
```bash
# Expo Atlasを使用したバンドル分析
EXPO_ATLAS=true pnpm -C apps/sandbox start

# ソースマップ付きエクスポート（分析用）
pnpm -C apps/sandbox expo export --source-maps

# 未使用翻訳の検出（カスタムスクリプト作成予定）
pnpm -C apps/sandbox analyze:translations
```

### 技術的な考慮事項
- React Nativeでは言語別の動的インポート（コードスプリッティング）に制限がある
- Metro Transformerを使用しているため、.poファイルは既にバンドルに含まれる
- 最適化の主なアプローチ：メッセージレベルの遅延読み込み、メモ化、キャッシング
