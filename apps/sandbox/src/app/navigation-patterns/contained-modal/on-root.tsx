      ...
      <PresentationSampleScreen
        ...
        iosBehavior={
          <Trans>
            ...
            見た目の差がなくなり、containedModal を使う意味が消える ── 本来は tab content の bounds
            に収まることを期待する presentation 値なので、ルート配置は推奨されない (観察用)。swipe
            down では閉じない。
          </Trans>
        }
        androidBehavior={
          <Trans>
            ...
            と同等の挙動になる。`animation` 未指定なら OS 標準の遷移 (多くは右からスライド) になる。
          </Trans>
        }
        dismissNote={
          <Trans>
            iOS では「閉じる」ボタンが必須 (swipe down は効かない)。Android では戻る操作でも閉じる。
            **ルート版は fullScreenModal とほぼ同じ挙動になりフォールバック相当**。挙動差を
            観察したい場合は in-tab 版と並べて比較する。
          </Trans>
        }
      />
    </>
